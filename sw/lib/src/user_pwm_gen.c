...
int user_pwm_set_control(uint32_t instance, UserPWMMode mode, UserPWMClkDiv clkdiv){
  ...
  *reg32(USER_PWM_ADDRESS, instance<<6 | USER_PWM_CONTROL_REG_OFFSET) = data;
  return 0;
}

int user_pwm_set_counter_top(uint32_t instance, uint32_t counter_top){
  *reg32(USER_PWM_ADDRESS, instance<<6 | USER_PWM_CNT_TOP_REG_OFFSET) = counter_top;
}

int user_pwm_set_counter_compare(uint32_t instance, uint32_t counter_compare){
  *reg32(USER_PWM_ADDRESS, instance<<6 | USER_PWM_CNT_COMP_REG_OFFSET) = counter_compare;
}